    ...
    const fetchMembers = async () => {
      ...
      try {
        const response = await databases.listDocuments(
            ...
            'members', // Replace with your members collection ID
            [
                Query.equal("gymOwnerId", gymOwnerId)
            ]
        );
        const membersList = response.documents.map(doc => {
            const data = doc;
  ...
  const handleAddMember = async (values: z.infer<typeof memberSchema>) => {
    ...
    try {
        ...
        const addedMember: Member = { 
            id: response.$id, 
            ...newMemberData,
            startDate: new Date(newMemberData.startDate),
            endDate: new Date(newMemberData.endDate)
        };
                                        ...
                                        <FormItem className="space-y-3">
                                            ...
                                            <FormControl>
                                                <RadioGroup onValueChange={field.onChange} defaultValue={field.value} className="flex flex-col space-y-2">
                                                    ...
                                                    <FormItem className="flex items-center space-x-3 space-x-reverse"><FormControl><RadioGroupItem value="Monthly" /></FormControl><FormLabel className_="font-normal">شهري</FormLabel></FormItem>
                                                </RadioGroup>
                                            </FormControl>
                                            <FormMessage />
                                        </FormItem>
